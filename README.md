...
- `relationships _ArrayRef_`
    ...
    Specifying `relationships` will create additional routes that fall under the resource. These URLs
    ...
    both will point to the same controller action. So in essence, `/api/posts/1/relationships/author` and
    `/api/posts/1/author` will go to the action `{http_method}_related_author`. This is because in my
